...
inline FragmentCommonData FragmentSetup (float4 i_tex, half3 i_eyeVec, half3 i_normalWorld, half3 i_viewDirForParallax, half3x3 i_tanToWorld, half3 i_posWorld)
{
	...
	// if the face is viewed from the back, invert the normal
	// this is done on the normale before normal map is applied to avoid artefacts
	if(dot(eyeVec, i_normalWorld) > 0) normalWorld *= -1;
